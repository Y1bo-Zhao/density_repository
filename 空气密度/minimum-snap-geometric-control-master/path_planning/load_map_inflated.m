function map = load_map_inflated(filename, xy_res, z_res, margin)
...
%             boundary = textscan(tline(9:end));
        end
        if strncmpi(tline, 'block', 5)
            block = strread(tline(6:end));
%             block = textscan(tline(6:end));
...
blocks(:,1:2) = blocks(:,1:2)-.275;
blocks(:,3) = blocks(:,3)-.15;
blocks(:,4:5) = blocks(:,4:5)+.275;
blocks(:,6) = blocks(:,6)+.15;
map.blocks = blocks;
map.xy_res = xy_res;
map.z_res = z_res;
